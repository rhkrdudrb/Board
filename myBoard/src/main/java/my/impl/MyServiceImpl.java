...
public class MyServiceImpl extends EgovAbstractServiceImpl implements MyService {
	...
	public ArrayList<MyVo> paymentDetailsq(MyVo mv) {
		return MyMapper.paymentDetailsq(mv);
	}
	...
	public String stateUpdate(MyVo mv) {
		...
		System.out.println(mv.getApvsq());
		System.out.println(resultChk);
		System.out.println(resultlastChk);
	...
	public String companionUpdate(MyVo mv) {
		...
		String resultChk = MyMapper.stateUpdateCheck(mv);
		System.out.println(resultChk);
		if (resultChk == null || resultChk.equals("B") || resultChk.equals("A") || resultChk.equals("S")) {
			// 반려한 사람 상태값 변경 ---->결재디테일
	...
	public ArrayList<MyVo> getState(MyVo mv) {
		return MyMapper.getState(mv);
	}
}